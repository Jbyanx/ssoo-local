...
int main(void) {
    ...
    // Bucle para crear dos procesos hijos.
    for (int i = 0; i < 2; i++) {
        if (!fork()) { // Si es un proceso hijo...
                for (int j = 0; j < 2; j++) //durante dos veces
                {
                    if(!fork()){ //creare hijos pero el hijo no hace nada
                        break;
                    }
                }
            break;
    ...
    if (root == getpid()) {
        ...
        // Espera a que terminen los dos hijos directos.
        for (int k = 0; k < 2; k++) {
